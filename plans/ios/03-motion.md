...
|---|---|---|
| App open (icon → app) | r 0.42 ζ 0.86 | Causality: the app *is* that icon |
| App close (app → icon) | r 0.50 ζ 0.80 | Spatial: where it lives |
| Interactive Home / switcher | finger-driven; release projection `v × 0.499`; settle r 0.45 ζ 0.85 | Direct manipulation |
| Folder open / close | r 0.42 ζ 0.86 / r 0.50 ζ 0.80 | Same physics as apps |
| Sheet present / dismiss | r 0.38 ζ 1.0 (no overshoot); finger-driven dismiss | Hierarchy |
| Control Center / Spotlight reveal | finger-driven; settle r 0.38 ζ 0.85–1.0 | Direct manipulation |
| Banner in | r 0.45 ζ 0.78 | Peripheral arrival |
...
| Interaction | Spec |
|---|---|
| Nav push / pop | 350 ms `cubic-bezier(0.32, 0.72, 0, 1)`; underlay parallax −30 % + dim 10 %; interactive on edge swipe |
| Large-title collapse | scroll-linked, transform-only |
| Press dim | 80 ms in / 200 ms out |
| Banner out | 250 ms ease-in (or velocity hand-off on swipe) |
| Status-bar style crossfade | 200 ms |
| Home arrival fly-in | icons 1.15 → 1 + fade, 12 ms column stagger, spring r 0.5 ζ 0.9 |
| Safari bar collapse | 250 ms `0.32, 0.72, 0, 1` |
