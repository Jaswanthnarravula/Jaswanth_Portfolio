...
| Speed, second pass (2026-09-23, owner: "opening significantly very slow") | Real mouse clicks, headed Chromium, 1440 × 900: an open lands in 281–304 ms with 15–17 frames (was 374–447 ms, 7–8 frames at ~57 ms); Home in 293 ms at 60 fps (was 429 ms, 9 frames). Cause: repaint of the gradient wallpaper and Home Screen every frame, fixed with layers during flights (`plans/ios/03-motion.md` "Layers during a flight"). Evidence: `ios.spec.ts` 121 passed · 0 failed (chromium-desktop, iphone, reduced-motion, asset-original) and the 4 iOS `perf` tests green on builds `.next-perf-final` / `.next-perf-orig`. Slow-CPU follow-up in the roadmap Backlog |
| Wallpaper (2026-09-23) | Official mode shows the iPhone 16 wallpaper (Ultramarine, one 57.7 KB AVIF, owner's choice); original mode keeps the gradient. `IOS-ID-03 · ASSET-MODE-01` e2e green in both modes |
