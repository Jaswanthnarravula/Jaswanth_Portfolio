...
export interface BadgeInput {
  ...
  /** Short side ≥ 600 px counts as expanded; a phone in either orientation is compact. */
 ...
 * fine + expanded → macOS; nothing else. Only a visible (released) OS can carry it.
 */
export function suitedOs({ pointer, compact, seed, visible }: BadgeInput): OsId | null {
  let pick: OsId | null = null;
  if (pointer === 'coarse' && compact) {
  ...
  } else if (pointer === 'fine' && !compact) pick = 'macos';
